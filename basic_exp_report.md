...
银行卡类型为IC卡，存储的数据是经过一定处理的，需要查阅一定资料才能读取，安全性比较高。
...
天津公交单程卡是M1卡，可以读取卡片内存中的部分数据，但无法进行写入，安全性尚可，相对北京交通一卡通安全性较低。
