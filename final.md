...
​	校园一卡通系统的基本功能参考清华校园一卡通系统即可：

​		创建新卡的时候在数据库中将卡的uid和学生绑定起来即可，删除旧卡将卡的uid和学生解绑即可。这样卡上的余额和卡本身也是分离的，不会因为旧卡丢失或者损坏就损失其中的余额。

​		更新有效期只要通过卡的uid识别学生，然后将为对应的学生注册有效期即可，和卡是无关的。

​		门禁系统只需要在后台维护允许通过的用户列表，门禁机只需要用卡的uid去向后台查询是否许可即可。

​		闪付储值与消费同样只是访问和修改后台的数据库，不在卡上保存信息。

​	显而易见，读卡器和后台数据库是这个系统不可或缺的部分。如果不使用统一的中央数据库而将各种信息在卡上保存，一方面很难处理丢失卡，损坏卡等问题，另一方面无法进行在线充值，在线挂失这样方便实用的操作。由于读卡器是嵌入式设备，进行网络访问，用户交互等比较难以实现，所以我们添加了一个桌面终端，主要负责连接用户，读卡器和后台。我们可以将桌面终端和读卡器类比为清华校园中的圈存机等设备。

​	因此校园一卡通系统的构建大致可以分为三个部分，读取卡片信息的读卡器、负责与用户交互的终端设备和负责存储信息和进行各种查询的网络后台。考虑到校园卡存在丢失等情况，其中的信息存在被破解的可能，出于安全考虑，此系统中网络后台负责存储全部的用户信息。而另一方面，为了方便起见，此系统允许用户将少量的钱转储到在卡片上存储的“小钱包”上，这个小钱包是不需要终端设备联网的。同时，在终端设备临时断网的时候，我们也允许门禁系统放行最近通过过的少量用户。
...
​	2.桌面终端程序打包为exe文件，隐藏python源代码，大大加强逆向工程的难度。

​	3. 在外存中保存的access log等信息使用AES加密和随机的无意义密钥。
...
1. 与用户的交互
   ...
   我们通过python3-tk实现了一个GUI，用于和用户的操作。GUI设计如下：

   ![login](img/login.png)![terminal](img/terminal.png)

2. 与网站后台的交互

   简单来说使用python  requests访问后台暴露出的接口即可。这些接口在下一节中详细列举。

   为了获得访问权限，每次访问API都需要附加JWT token: `requests.post(url, json={"card_id": uid}, params={"jwt": self.token}, timeout=0.5, verify=False)`

   token在登录的时候获取，获取token需要使用账号和密码登录：`requests.post(url, data={"username": user_name, "password": password}, verify=False)`

   账号和密码很少上传，而且即使上传也是使用HTTPS协议加密的POST请求，很难泄露。而token虽然每次请求都会上传，而且公开在query string中，但是token本身无意义，是随机生成的，且定期失效，并且由于HTTPS协议的特性，中间链路基本不可能截取到token，所以也是安全的。

3. 与读卡器的交互

   与读卡器的交互使用串口通信，目前使用的是USB线连接，但是实际上使用蓝牙模块或者WiFi模块也都可以。我们规定了一系列换行符分割的命令用来执行终端和读卡器的交互，在上一节中已经详细列出，不再赘述。

   由于读卡器本身运行速度慢，而且串口通信带宽低，所以串口通信和GUI的刷新是*异步*的。这样做主要是为了保证GUI流畅的用户体验。

一般来讲，桌面终端的所有操作都是需要和后台交互的，这些操作的安全性都是有保障的。唯二不需要后台操作的是小钱包消费和临时断网场景下的门禁系统。

在我们的系统中，临时断网场景的下的门禁系统只适用于宿舍楼之类安全级别不高的场所，而实验室，机房等高安全级别的场所不应该开启此功能。如果开启此功能，在登录之后如果向后台查询门禁信息失败，会从本地数据库中查询用户记录。本地数据库缓存了最近若干天的最多若干个被允许通过门禁的用户。如此，有的时候网络不稳定，也不会影响学生的出入。这种设计的安全性有以下几方面保证：

       	1. 首先，能够被缓存到本地数据库中的用户都是最近几天成功联网查询门禁通过的用户。存在的唯一漏洞仅为校园卡刚刚失效的用户还能够进入门禁，但是这一点对于类似宿舍楼这样的场景来说无伤大雅。
     	2. 在终端中存储的本地数据库是使用AES加密保存的，即便盗取了刷卡器，也需要解密本地数据库的加密（不可行），或者对终端程序逆向工程。
...
我们后台的代码部署在了一台腾讯云的服务器上， 使用Apache2.4,配置了域名解析和SSL，只支持https访问。同时为此服务器配置了防火墙规则，大大提高了安全性。此网站一段时间内都可以访问，直接在浏览器中访问还可以通过前端界面管理数据库。
...
直接双击运行即可（需要电脑上已连接一个读卡器到某一个串口设备）
...
![invalid_card](img/invalid_card.png)

在显示`Id`处输入学号（要求该学号在后台教务系统已存在）。点击`New Card`，将修改卡的密钥。点击`Create`，将把此卡与学号绑定。效果如下：

![new_created](img/new_created.png)

无效的学号点击之后不会有任何反应。

#### 删除旧卡

点击`Delete`,将接触此卡与对应学号的绑定：

![delete](img/delete.png)

与此同时卡上的密钥将被置为默认密钥，小钱包将被清空。

#### 更新有效期

目前更新有效期将直接将卡的有效期增加18周,点击`Update`即可更新有效期
...
如果终端可以连接到后台，那么将卡放到读卡器上的时候会向后台请求查询，成功会亮绿灯，失败会亮红灯。成功的时候会将用户信息加入到本地缓存中，本地缓存有最大时间和最大用户数的限制。

如果终端网络连接在登录后异常断开，那么会查询本地的缓存数据库。无论成功还是失败都不会更新本地数据库。

#### 充值与消费

充值与消费需要和后台交互，由后台控制金额的检查。点击`Consume`,`Recharge`将按照金额输入框中的进行充值和消费。成功会更新余额，失败则不会有任何变化

消费之前：

![before](img/after_update.png)

消费了123元之后：

![after](img/consume.png)
...
![before](img/recharge.png)

小钱包充值之后：

![after](img/small_recharge.png)

小钱包消费之后：

![after](img/consume_small.png)

## 五、分析与讨论

我们再次讨论这个系统的安全性和便利性的问题。

读卡器和桌面终端在实际的使用中是封装为类似清华校园圈存机那样的设备使用的，一般人如果想要和其交互只能通过暴露出来的有限的按钮和屏幕，不可能实施更多的攻击。如果想要进一步侵入，要么只能破坏设备外壳，要么只能篡改后台发往终端的数据。首先后者是不可行的，因为我们只通过SSL加密过的https协议传输数据。而前者一方面实现难度较大，容易引起注意，另一方面即使破坏了，也还是需要逆向工程以及解密的。

后台由于使用了成熟的http服务器和应用框架，对于一般的目录遍历，SQL注入，跨站脚本攻击等常见的web攻击方式都有很好的防护能力。

而卡上信息使用密钥保护，本身就是不可破解的，即使有人拿到卡，不知道密钥也无法读写。即便恰好破解了，我们卡上的小钱包信息也是AES加密保存的，还需要破解一个长为32的完全随机的密钥，基本不可行。

对于便利性：由于我们本地有小钱包系统，所以在饮水机，淋浴间等地方部署水卡澡卡等的刷卡器的时候只需要可以省去网络设备的成本。门禁系统我们将数据放在后台，这样可以很方便地通过修改后台数据库配置不同地方的门禁系统（比如为宿舍楼，实验室等），而不需要去一个个修改刷卡机内的数据。同理，由于绝大部分数据都在后台，我们可以很方便地实现在线充值，在线挂失等实用的功能。
